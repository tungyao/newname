...
func GetRandomName(n int) (name string) {
...
var numberFlag = flag.Int("number",100, "需要名字个数 : -number")
var lengthFlag = flag.Int("length",3, "需要名字长度 : -length")
func main() { 
	flag.Parse()
	var m string
	for i:=0;i<*numberFlag;i++  {
		s := GetRandomName(*lengthFlag)
		fmt.Println(s)
		if i % 10 !=0 {
			m += s +"\t"
		}else{
